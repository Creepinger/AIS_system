...
| 角色 | 代号 | 核心职责 |
|------|------|---------|
| 组长 | Core | 协议解析引擎(6-bit 解码、消息 1/18/5 解析、接口契约) |
...
- **协议解析** —— NMEA 0183 拆包 + BCC 校验 + 多帧拼接 + 消息 1/18/5
- **实时推送** —— FastAPI WebSocket 每 200ms 广播船舶快照与统计
- **Web 控制** —— 暂停/继续/清空/换源/上传文件 全部可从浏览器触发
...
├── docs/
│   ├── interface_contract.md     # 接口契约(含 WebSocket 通道 v1.1)
